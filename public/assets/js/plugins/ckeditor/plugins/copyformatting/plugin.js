...
*/
(function () {
  function k (a, b, e, d) { var c = new CKEDITOR.dom.walker(a); if (a = a.startContainer.getAscendant(b, !0) || a.endContainer.getAscendant(b, !0)) if (e(a), d) return; for (;a = c.next();) if (a = a.getAscendant(b, !0)) if (e(a), d) break } function u (a, b) { var e = { ul: 'ol', ol: 'ul' }; return l(b, function (b) { return b.element === a || b.element === e[a] }) !== -1 } function q (a) {
...
'styles/copyformatting.css'); a.copyFormatting = new b.state(a); a.addCommand('copyFormatting', b.commands.copyFormatting); a.addCommand('applyFormatting', b.commands.applyFormatting); a.ui.addButton('CopyFormatting', { label: a.lang.copyformatting.label, command: 'copyFormatting', toolbar: 'cleanup,0' }); a.on('contentDom', function () {
        var b = a.getCommand('copyFormatting'); var d = a.editable(); var c = d.isInline() ? d : a.document; var f = a.ui.get('CopyFormatting'); d.attachListener(c, 'mouseup', function (d) {
...
this.editor.config.copyFormatting_allowedContexts; return !0 === b || l(b, a) !== -1
    ...
    commands: {
      copyFormatting: {
        exec: function (a, b) {
          var e = CKEDITOR.plugins.copyformatting; var d = a.copyFormatting; var c = b ? b.from == 'keystrokeHandler' : !1; var f = b ? b.sticky || c : !1; var g = e._getCursorContainer(a); var h = CKEDITOR.document.getDocumentElement(); if (this.state === CKEDITOR.TRISTATE_ON) {
            return d.styles = null, d.sticky = !1, g.removeClass('cke_copyformatting_active'), h.removeClass('cke_copyformatting_disabled'), h.removeClass('cke_copyformatting_tableresize_cursor'),
...
!CKEDITOR.env.edge ? !1 : !0,
    ...
    _convertElementToStyleDef: function (a) { var b = CKEDITOR.tools; var e = a.getAttributes(CKEDITOR.plugins.copyformatting.excludedAttributes); var b = b.parseCssText(a.getAttribute('style'), !0, !0); return { element: a.getName(), type: CKEDITOR.STYLE_INLINE, attributes: e, styles: b } },
    _extractStylesFromElement: function (a, b) { var e = {}; var d = []; do if (b.type === CKEDITOR.NODE_ELEMENT && !b.hasAttribute('data-cke-bookmark') && (e.element = b, a.copyFormatting.fire('extractFormatting', e, a) && e.styleDef && d.push(new CKEDITOR.style(e.styleDef)), b.getName && l(CKEDITOR.plugins.copyformatting.breakOnElements, b.getName()) !== -1)) break; while ((b = b.getParent()) && b.type === CKEDITOR.NODE_ELEMENT);return d },
    ...
    _removeStylesFromElementInRange: function (a, b) { for (var e = l(['ol', 'ul', 'table'], b) !== -1, d = new CKEDITOR.dom.walker(a), c; c = d.next();) if (c = c.getAscendant(b, !0)) if (c.removeAttributes(c.getAttributes()), e) break },
    _getSelectedWordOffset: function (a) {
      function b (a, b) { return a[b ? 'getPrevious' : 'getNext'](function (a) { return a.type !== CKEDITOR.NODE_COMMENT }) } function e (a) { return a.type == CKEDITOR.NODE_ELEMENT ? (a = a.getHtml().replace(/<span.*?>&nbsp;<\/span>/g, ''), a.replace(/<.*?>/g, '')) : a.getText() } function d (a,
        ...
        } else if (c && (n += 1, n > h.length)) return d(f); return { node: f, offset: n }
    ...
    _filterStyles: function (a) {
      var b = CKEDITOR.tools.isEmpty; var e = []; var d; var c; for (c = 0; c <
a.length; c++)d = a[c]._.definition, CKEDITOR.tools.indexOf(CKEDITOR.plugins.copyformatting.inlineBoundary, d.element) !== -1 && (d.element = a[c].element = 'span'), d.element === 'span' && b(d.attributes) && b(d.styles) || e.push(a[c]); return e
    },
    _determineContext: function (a) { function b (b) { var d = new CKEDITOR.dom.walker(a); var c; if (a.startContainer.getAscendant(b, !0) || a.endContainer.getAscendant(b, !0)) return !0; for (;c = d.next();) if (c.getAscendant(b, !0)) return !0 } return b({ ul: 1, ol: 1 }) ? 'list' : b('table') ? 'table' : 'text' },
    _applyStylesToTextContext: function (a,
      b, e) { var d = CKEDITOR.plugins.copyformatting; var c = d.excludedAttributesFromInlineTransform; var f; var g; CKEDITOR.env.webkit && !CKEDITOR.env.chrome && a.getSelection().selectRanges([b]); for (f = 0; f < e.length; f++) if (b = e[f], l(d.excludedElementsFromInlineTransform, b.element) === -1) { if (l(d.elementsForInlineTransform, b.element) !== -1) for (b.element = b._.definition.element = 'span', g = 0; g < c.length; g++)b._.definition.attributes[c[g]] && delete b._.definition.attributes[c[g]]; b.apply(a) } },
    ...
    _applyFormat: function (a, b) {
      var e = a.getSelection().getRanges()[0]; var d = CKEDITOR.plugins.copyformatting; var c; var f; if (!e) return !1; if (e.collapsed) {
        f =
