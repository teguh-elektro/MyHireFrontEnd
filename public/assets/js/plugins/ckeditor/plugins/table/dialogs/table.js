...
*/
(function () {
  function w (a) { for (var f = 0, p = 0, n = 0, q, e = a.$.rows.length; n < e; n++) { q = a.$.rows[n]; for (var d = f = 0, b, c = q.cells.length; d < c; d++)b = q.cells[d], f += b.colSpan; f > p && (p = f) } return p } function t (a) { return function () { var f = this.getValue(); var f = !!(CKEDITOR.dialog.validate.integer().call(this, f) && f > 0); f || (alert(a), this.select()); return f } } function r (a, f) {
    var p = function (e) { return new CKEDITOR.dom.element(e, a.document) }; var r = a.editable(); var q = a.plugins.dialogadvtab; return {
      ...
      onShow: function () {
        var e = a.getSelection(); var d = e.getRanges(); var b; var c = this.getContentElement('info', 'txtRows'); var h = this.getContentElement('info', 'txtCols'); var u = this.getContentElement('info', 'txtWidth'); var l = this.getContentElement('info',
          'txtHeight'); f == 'tableProperties' && ((e = e.getSelectedElement()) && e.is('table') ? b = e : d.length > 0 && (CKEDITOR.env.webkit && d[0].shrink(CKEDITOR.NODE_ELEMENT), b = a.elementPath(d[0].getCommonAncestor(!0)).contains('table', 1)), this._.selectedElement = b); b ? (this.setupContent(b), c && c.disable(), h && h.disable()) : (c && c.enable(), h && h.enable()); u && u.onChange(); l && l.onChange()
      },
      onOk: function () {
        var e = a.getSelection(); var d = this._.selectedElement && e.createBookmarks(); var b = this._.selectedElement || p('table'); var c = {}; this.commitContent(c,
          b); if (c.info) {
          c = c.info; if (!this._.selectedElement) for (var h = b.append(p('tbody')), f = parseInt(c.txtRows, 10) || 0, l = parseInt(c.txtCols, 10) || 0, k = 0; k < f; k++) for (var g = h.append(p('tr')), m = 0; m < l; m++)g.append(p('td')).appendBogus(); f = c.selHeaders; if (!b.$.tHead && (f == 'row' || f == 'both')) {
            g = b.getElementsByTag('thead').getItem(0); h = b.getElementsByTag('tbody').getItem(0); l = h.getElementsByTag('tr').getItem(0); g || (g = new CKEDITOR.dom.element('thead'), g.insertBefore(h)); for (k = 0; k < l.getChildCount(); k++) {
              h = l.getChild(k),
              h.type != CKEDITOR.NODE_ELEMENT || h.data('cke-bookmark') || (h.renameNode('th'), h.setAttribute('scope', 'col'))
            }g.append(l.remove())
          } if (b.$.tHead !== null && f != 'row' && f != 'both') { g = new CKEDITOR.dom.element(b.$.tHead); for (h = b.getElementsByTag('tbody').getItem(0); g.getChildCount() > 0;) { l = g.getFirst(); for (k = 0; k < l.getChildCount(); k++)m = l.getChild(k), m.type == CKEDITOR.NODE_ELEMENT && (m.renameNode('td'), m.removeAttribute('scope')); h.append(l, !0) }g.remove() } if (!this.hasColumnHeaders && (f == 'col' || f == 'both')) {
            for (g = 0; g <
b.$.rows.length; g++)m = new CKEDITOR.dom.element(b.$.rows[g].cells[0]), m.renameNode('th'), m.setAttribute('scope', 'row')
          } if (this.hasColumnHeaders && f != 'col' && f != 'both') for (k = 0; k < b.$.rows.length; k++)g = new CKEDITOR.dom.element(b.$.rows[k]), g.getParent().getName() == 'tbody' && (m = new CKEDITOR.dom.element(g.$.cells[0]), m.renameNode('td'), m.removeAttribute('scope')); c.txtHeight ? b.setStyle('height', c.txtHeight) : b.removeStyle('height'); c.txtWidth ? b.setStyle('width', c.txtWidth) : b.removeStyle('width'); b.getAttribute('style') ||
b.removeAttribute('style')
        } if (this._.selectedElement) try { e.selectBookmarks(d) } catch (n) {} else a.insertElement(b), setTimeout(function () { var e = new CKEDITOR.dom.element(b.$.rows[0].cells[0]); var c = a.createRange(); c.moveToPosition(e, CKEDITOR.POSITION_AFTER_START); c.select() }, 0)
      },
      contents: [{
        id: 'info',
        ...
        elements: [{
          ...
          children: [{
            ...
            children: [{
              ...
              items: [[a.lang.table.headersNone, ''], [a.lang.table.headersRow, 'row'],
...
'')
            ...
            children: [{
              ...
              children: [{
                ...
                default: a.filter.check('table{width}') ? r.getSize('width') < 500 ? '100%' : 500 : 0,
                getValue: v,
...
'')
  ...
  } var v = CKEDITOR.tools.cssLength; var n = function (a) { var f = this.id; a.info || (a.info = {}); a.info[f] = this.getValue() }; CKEDITOR.dialog.add('table', function (a) { return r(a, 'table') }); CKEDITOR.dialog.add('tableProperties', function (a) { return r(a, 'tableProperties') })
})()
