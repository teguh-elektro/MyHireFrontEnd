...
class BePagesCryptoDashboard {
  ...
  static initChartsCrypto () {
    ...
    const chartBitcoinCon = jQuery('.js-chartjs-bitcoin')
    const chartEthereumCon = jQuery('.js-chartjs-ethereum')
    const chartLitecoinCon = jQuery('.js-chartjs-litecoin')

    // Helper Classes
    let chartBitcoin, chartEthereum, chartLitecoin

    // Set up labels
    const chartCryptolabels = []
    for (i = 0; i < 30; i++) {
      chartCryptolabels[i] = (i === 29) ? '1 day ago' : (30 - i) + ' days ago'
    }

    // Cryto Data
    const chartBitcoinData = [10500, 10400, 9500, 8268, 10218, 8250, 8707, 9284, 9718, 9950, 9879, 10147, 10883, 11071, 11332, 11584, 11878, 13540, 16501, 16007, 15142, 14869, 16762, 17276, 16808, 16678, 16771, 12900, 13100, 14000]
    const chartEthereumData = [500, 525, 584, 485, 470, 320, 380, 580, 620, 785, 795, 801, 799, 750, 900, 920, 930, 1300, 1250, 1150, 1365, 1258, 980, 870, 860, 925, 999, 1050, 1090, 1100]
    const chartLitecoinData = [300, 320, 330, 331, 335, 340, 358, 310, 220, 180, 190, 195, 203, 187, 198, 258, 270, 340, 356, 309, 218, 230, 242, 243, 250, 210, 205, 226, 214, 250]
