...
class BeFormWizard {
  ...
  static initWizardSimple () {
    jQuery('.js-wizard-simple').bootstrapWizard({
      onTabShow: (tab, navigation, index) => {
        const percent = ((index + 1) / navigation.find('li').length) * 100

        // Get progress bar
        const progress = navigation.parents('.block').find('[data-wizard="progress"] > .progress-bar')
  ...
  static initWizardValidation () {
    // Get forms
    const formClassic = jQuery('.js-wizard-validation-classic-form')
    const formMaterial = jQuery('.js-wizard-validation-material-form')
    ...
    const validatorClassic = formClassic.validate({
    ...
    const validatorMaterial = formMaterial.validate({
      errorClass: 'invalid-feedback animated fadeInDown',
      errorElement: 'div',
      errorPlacement: (error, e) => {
        jQuery(e).parents('.form-group').append(error)
