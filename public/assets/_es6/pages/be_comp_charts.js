...
class BeCompCharts {
  ...
  static initChartsSparkline () {
    // Chart Containers
    const slcLine1 = jQuery('.js-slc-line1')
    const slcLine2 = jQuery('.js-slc-line2')
    const slcLine3 = jQuery('.js-slc-line3')
    const slcBar1 = jQuery('.js-slc-bar1')
    const slcBar2 = jQuery('.js-slc-bar2')
    const slcBar3 = jQuery('.js-slc-bar3')
    const slcPie1 = jQuery('.js-slc-pie1')
    const slcPie2 = jQuery('.js-slc-pie2')
    const slcPie3 = jQuery('.js-slc-pie3')
    const slcTristate1 = jQuery('.js-slc-tristate1')
    const slcTristate2 = jQuery('.js-slc-tristate2')
    const slcTristate3 = jQuery('.js-slc-tristate3')
  ...
  static initChartsChartJS () {
    ...
    const chartLinesCon = jQuery('.js-chartjs-lines')
    const chartBarsCon = jQuery('.js-chartjs-bars')
    const chartRadarCon = jQuery('.js-chartjs-radar')
    const chartPolarCon = jQuery('.js-chartjs-polar')
    const chartPieCon = jQuery('.js-chartjs-pie')
    const chartDonutCon = jQuery('.js-chartjs-donut')
  ...
  static initChartsFlot () {
    ...
    const flotLive = jQuery('.js-flot-live')
    const flotLines = jQuery('.js-flot-lines')
    const flotStacked = jQuery('.js-flot-stacked')
    const flotPie = jQuery('.js-flot-pie')
    const flotBars = jQuery('.js-flot-bars')

    // Demo Data
    const dataEarnings = [[1, 1500], [2, 1700], [3, 1400], [4, 1900], [5, 2500], [6, 2300], [7, 2700], [8, 3200], [9, 3500], [10, 3260], [11, 4100], [12, 4600]]
    const dataSales = [[1, 500], [2, 600], [3, 400], [4, 750], [5, 1150], [6, 950], [7, 1400], [8, 1700], [9, 1800], [10, 1300], [11, 1750], [12, 2900]]

    const dataSalesBefore = [[1, 500], [4, 600], [7, 1000], [10, 600], [13, 800], [16, 1200], [19, 1500], [22, 1600], [25, 2500], [28, 2700], [31, 3500], [34, 4500]]
    const dataSalesAfter = [[2, 900], [5, 1200], [8, 2000], [11, 1200], [14, 1600], [17, 2400], [20, 3000], [23, 3200], [26, 5000], [29, 5400], [32, 7000], [35, 9000]]

    const dataMonths = [[1, 'Jan'], [2, 'Feb'], [3, 'Mar'], [4, 'Apr'], [5, 'May'], [6, 'Jun'], [7, 'Jul'], [8, 'Aug'], [9, 'Sep'], [10, 'Oct'], [11, 'Nov'], [12, 'Dec']]
    const dataMonthsBars = [[2, 'Jan'], [5, 'Feb'], [8, 'Mar'], [11, 'Apr'], [14, 'May'], [17, 'Jun'], [20, 'Jul'], [23, 'Aug'], [26, 'Sep'], [29, 'Oct'], [32, 'Nov'], [35, 'Dec']]

    // Live Chart
    let dataLive = []; const y = 0; let chartLive

    function getRandomData () { // Random data generator
      if (dataLive.length > 0) { dataLive = dataLive.slice(1) }

      while (dataLive.length < 300) {
        const prev = dataLive.length > 0 ? dataLive[dataLive.length - 1] : 50
    ...
    if (flotLines.length) {
      ...
      let previousPoint = null; let ttlabel = null
      flotLines.bind('plothover', (event, pos, item) => {
        if (item) {
          if (previousPoint !== item.dataIndex) {
            ...
            const x = item.datapoint[0]; const y = item.datapoint[1]
